...
# Production deployment script for VOD Service
set -euo pipefail
...
echo -e "${BLUE}🚀 Deploying VOD Service...${NC}"
...
echo -e "${BLUE}🔄 Deploying Celery workers...${NC}"
kubectl rollout status deployment/"${SERVICE_NAME}-worker" -n "${NAMESPACE}" --timeout=300s
...
SERVICE_PORT=$(kubectl get service "${SERVICE_NAME}" -n "${NAMESPACE}" -o jsonpath='{.spec.ports[0].port}' 2>/dev/null || echo "8087")
...
if [[ "${SERVICE_URL}" == "localhost" ]]; then
    ...
    kubectl port-forward service/"${SERVICE_NAME}" 8087:8087 -n "${NAMESPACE}" &
    PORT_FORWARD_PID=$!
    sleep 5
...
echo -e "${GREEN}🎥 VOD Service deployed successfully!${NC}"
...
echo -e "${BLUE}🌸 Celery Flower: http://${SERVICE_URL}:5555${NC}"
...
echo "  Port forward: kubectl port-forward service/${SERVICE_NAME} 8087:8087 -n ${NAMESPACE}"
