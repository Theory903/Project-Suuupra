...
import { createLogger, expressLoggingMiddleware, LogLevel } from '../node';
import { OpenTelemetryManager, createOpenTelemetryMiddleware } from '../observability/opentelemetry';
...
app.use(createOpenTelemetryMiddleware(otelManager));

// Example: User service with comprehensive logging
class UserService {
  private logger = logger.withContext({ component: 'UserService' });
  ...
  async getUser(userId: string, requestId: string) {
    ...
    try {
      ...
      contextLogger.error('Failed to fetch user', error as Error, {
        userId,
        operation: 'getUser',
      });
  ...
  async createUser(userData: any, requestId: string) {
    ...
    try {
      ...
      contextLogger.error('Failed to create user', error as Error, {
        operation: 'createUser',
      });
...
app.get('/users/:id', async (req, res) => {
  ...
  try {
    ...
    requestLogger.error('Failed to get user', error as Error, {
      userId: req.params.id,
      statusCode: 500,
    });
...
app.post('/users/batch', async (req, res) => {
  ...
  try {
    ...
    requestLogger.error('Batch operation failed', error as Error, {
      operation: 'batchCreateUsers',
    });
